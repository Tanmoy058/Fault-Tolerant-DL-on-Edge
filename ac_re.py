...
# accumulators for each parameter in each network
def init_accums(ac_learner):  # in rmsprop
    accums = []
    params = ac_learner.get_params()
    for param in params:
...
# concat all state observations across trajectories
...
def get_traj_worker(ac_learner, env, pa, result):

    trajs = []

    for i in xrange(pa.num_seq_per_batch):
        traj = get_traj(ac_learner, env, pa.episode_max_length)
        trajs.append(traj)
...
def launch(pa, ac_resume=None, render=False, repre='image', end='no_new_job'):
    ...
    for ex in xrange(pa.batch_size + 1):  # last worker for updating the parameters
        ...
        if ac_resume is not None:
            policy_net_handle = open(ac_resume + "_policy.pkl", 'rb')
            policy_net_params = cPickle.load(policy_net_handle)

            value_net_handle = open(ac_resume + "_value.pkl", 'rb')
            value_net_params = cPickle.load(value_net_handle)

            ac_learner.set_net_params(policy_net_params, value_net_params)

        ac_learners.append(ac_learner)

    accums = init_accums(ac_learners[pa.batch_size])
    ...
    for iteration in xrange(1, pa.num_epochs):
        ...
        for ex in xrange(pa.num_ex):
            ...
            p = Process(target=get_traj_worker,
                        args=(ac_learners[ex_counter], envs[ex_idx], pa, manager_result, ))
            ps.append(p)
            ...
            if ex_counter >= pa.batch_size or ex == pa.num_ex - 1:
                ...
                grads = ac_learners[pa.batch_size].get_grad(all_ob, all_action, all_adv)
        ...
        if iteration % pa.output_freq == 0:
            ...
            policy_param_file = open(pa.output_filename + '_' + str(iteration) + "_policy" + '.pkl', 'wb')
            value_param_file = open(pa.output_filename + '_' + str(iteration) + "_value" + '.pkl', 'wb')

            cPickle.dump(params[0], policy_param_file, -1)
            policy_param_file.close()

            cPickle.dump(params[1], value_param_file, -1)
            value_param_file.close()

            pa.unseen = True
            slow_down_cdf.launch(pa, ac_resume=(pa.output_filename + '_' + str(iteration) + '.pkl'),
...
def main():
    ...
    launch(pa, ac_resume=ac_resume, render=render, repre='image', end='all_done')
